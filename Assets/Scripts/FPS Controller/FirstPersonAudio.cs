...
public class FirstPersonAudio : MonoBehaviour {
  ...
  Vector2 CurrentCharacterPosition => new Vector2(
      character.transform.position.x, character.transform.position.z);
  ...
  void Reset() {
    ...
    groundCheck =
        (transform.parent ?? transform).GetComponentInChildren<GroundCheck>();
  ...
  void FixedUpdate() {
    if (Time.timeScale != 0) {
      ...
      float velocity =
          Vector3.Distance(CurrentCharacterPosition, lastCharacterPosition);
      if (velocity >= velocityThreshold && groundCheck &&
  ...
  void SetPlayingMovingAudio(AudioSource audioToPlay) {
    ...
    foreach (var audio in MovingAudios.Where(audio => audio != audioToPlay &&
                                                      audio != null)) {
      audio.Pause();
    }
...
#region Play instant - related audios.
  ...
  void PlayCrouchStartAudio() => PlayRandomClip(crouchStartAudio,
                                                crouchStartSFX);
...
#region Utility.
  ...
  AudioSource GetOrCreateAudioSource(string name) {
    ...
    AudioSource result = System.Array.Find(
        GetComponentsInChildren<AudioSource>(), a => a.name == name);
    if (result)
      return result;
  ...
  static void PlayRandomClip(AudioSource audio, AudioClip[] clips) {
    ...
    // Get a random clip. If possible, make sure that it's not the same as the
    // clip that is already on the audiosource.
