...
public class PathfindingGrid : MonoBehaviour {
  ...
  void Start() {
    gridWorldSize =
        GameObject.Find("Game").GetComponent<DungeonGenerator>().size * 12;
    ...
    foreach (TerrainType region in walkableRegions) {
      ...
      walkableRegionsDictionary.Add((int)Mathf.Log(region.terrainMask.value, 2),
                                    region.terrainPenalty);
    }
  }
  ...
  void BlurPenaltyMap(int blurSize) {
    ...
    for (int x = 0; x < gridSizeX; x++) {
      ...
      int blurredPenalty = Mathf.RoundToInt((float)penaltiesVerticalPass[x, 0] /
      ...
      for (int y = 1; y < gridSizeY; y++) {
        ...
        blurredPenalty = Mathf.RoundToInt((float)penaltiesVerticalPass[x, y] /
                                          (kernelSize * kernelSize));
